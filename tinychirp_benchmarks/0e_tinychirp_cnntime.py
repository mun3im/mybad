...
class TrainingConfig:
    ...
    output_dir: str = 'results_0e_tinychirp_cnntime'
    cache_dir: str = '/Volumes/Evo/cache_tinychirp_waveforms'
...
def build_cnn_time_model(
    ...
    # Block 1: Conv1D(3, filters=4) + ReLU
    x = tf.keras.layers.Conv1D(filters=4, kernel_size=3, padding="same")(inputs)
    x = tf.keras.layers.Activation("relu")(x)  # (48000, 4)

    # MaxPool
    x = tf.keras.layers.MaxPooling1D(pool_size=2)(x)  # (24000, 4)

    # Block 2: Conv1D(3, filters=8) + ReLU
    x = tf.keras.layers.Conv1D(filters=8, kernel_size=3, padding="same")(x)
    x = tf.keras.layers.Activation("relu")(x)  # (24000, 8)
    ...
    model = tf.keras.models.Model(inputs, outputs, name="TinyChirp_CNNTime")
...
def create_tf_dataset_from_cache(split: str, config: TrainingConfig,
    ...
    if augment:
        def augment_waveform(waveform, label):
            ...
            noise = tf.random.normal(tf.shape(waveform), mean=0.0, stddev=0.005)
            waveform = waveform + noise

            # Random time shift (circular shift up to 0.1 seconds = 1600 samples)
...
def save_config(config: TrainingConfig, output_dir: Path, args, system_info: dict):
    ...
    with open(config_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("TRAINING CONFIGURATION - TINYCHIRP CNN-TIME\n")
        f.write("="*60 + "\n\n")
...
def main():
    ...
    config.output_dir = f'results_0e_tinychirp_cnntime_r{config.random_seed}_{platform.system().lower()}'
    ...
    logger.info("="*60)
    logger.info("TinyChirp CNN-Time Training")
    logger.info("="*60)
    ...
    try:
        ...
        model = build_cnn_time_model(input_shape=(48000, 1), num_classes=2)
        ...
        with open(summary_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("TinyChirp CNN-Time Training Results Summary\n")
            f.write("="*60 + "\n\n")
